<template>
  <Notification
    @notification="notify"
    v-model:showNotification="showNotification"
    >Товар добавлен в корзину</Notification
  >
  <div class="result">
    <h2 v-if="cocktails_list.length > 0">
      видно {{ cocktails_list.length }} из {{ cocktails.length }}
    </h2>
  ...
  <div class="wrapper">
    <CocktailItem
      v-for="cocktail in cocktails_list"
      :key="cocktail.id"
      :cocktail="cocktail"
      @addToCart="addToCart"
    />
